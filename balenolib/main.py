"""Application entrypoint and main window for Baleno."""
...
from balenolib.constants import VERSION_LABEL
from balenolib.automation import AutomationTab
from balenolib.topology.tab import TopologyTab
from balenolib.network import (
...
from balenolib.terminal import (
    TerminalWidget, TerminalDialog, TerminalTabbedWindow,
    DetachedTerminalWindow, DetachableTabBar
)
from balenolib.ui.dialogs import StickyNoteDialog, VendorReferenceDialog, FileTextEditor
from balenolib.ui.profiles import SshProfilesTree, SerialProfilesTree
from balenolib.ui.widgets import FlatComboButton, _CollapsibleGroupBox
from balenolib.utils import (
...
class SerialTerminalGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Baleno v{VERSION_LABEL}")
    ...
    def _import_remmina(self):
        ...
        # Protocol mapping: Remmina name → Baleno name, default port
        SUPPORTED = {
            'SSH':    ('SSH',    '22'),
            'VNC':    ('VNC',    '5900'),
            'RDP':    ('RDP',    '3389'),
    ...
    def _show_about_dialog(self):
        ...
        dlg = QDialog(self)
        dlg.setWindowTitle("About Baleno")
        ...
        app_icon_path = self.get_icon_path('baleno-64.png')
        ...
        # Description
        desc = QLabel(
            "Baleno is an open-source network management GUI built with PyQt6.\n"
        ...
        photo_paths = [
            ...
            os.path.join(_appdir, 'usr/share/baleno/photo.png') if _appdir else '',
            '/usr/share/baleno/photo.png',
        ]
        photo_pix = None
    ...
    def create_filetransfer_page(self):
        ...
        self._ft_mode_client_btn.setToolTip(
            "<b>Client mode</b><br>"
            "Baleno connects to a remote server and transfers files.<br>"
        ...
        self._ft_mode_server_btn.setToolTip(
            "<b>Server mode</b><br>"
            "Baleno starts a local server (TFTP/FTP/SCP/SMB) on this machine.<br>"
        ...
        ssh_info = QLabel(
            ...
            "Baleno can show its status and start/stop it via systemctl."
        )
        ...
        self._smb_share_name = QLineEdit("baleno-share")
        ...
        self._smb_comment = QLineEdit("Baleno shared folder")
    ...
    def get_icon_path(self, icon_name):
        ...
        # the balenolib package directory.
        ...
        if getattr(sys, '_MEIPASS', None):
            ...
            if os.path.exists(bundle_path):
                return bundle_path
        # Check for Flatpak
        flatpak_path = f'/app/share/io.github.benjamimgois.baleno/icons/{icon_name}'
        if os.path.exists(flatpak_path):
            return flatpak_path
        # Check for AppImage
        if os.environ.get('APPDIR'):
            appdir = os.environ.get('APPDIR')
            appimage_path = os.path.join(appdir, f'usr/share/baleno/icons/{icon_name}')
            if os.path.exists(appimage_path):
                return appimage_path
        # Check for system installation (AUR, Debian, etc.)
        system_path = f'/usr/share/baleno/icons/{icon_name}'
    ...
    def get_vendor_icon_path(self, vendor):
        ...
        for base in [
            '/app/share/io.github.benjamimgois.baleno/vendors',
            os.path.join(os.environ.get('APPDIR', ''), 'usr/share/baleno/vendors'),
            os.path.join(os.path.dirname(__file__), 'assets/vendors'),
            '/usr/share/baleno/vendors',
        ]:
    ...
    def toggle_tftp_server(self):
        ...
        if self.tftp_process is not None:
            ...
            try:
                ...
                print(f"[Baleno] Starting TFTP server on {ip}:69")
                print(f"[Baleno] Directory: {directory}")
                ...
                print(f"[Baleno] TFTP server started successfully")
    ...
    def _ft_smb_apply_config(self):
        ...
        share      = self._smb_share_name.text().strip().replace(' ', '-') or 'baleno-share'
    ...
    def _launch_native_terminal(self, command_list):
        ...
        if not emulator:
            QMessageBox.warning(
                ...
                "Falling back to the Baleno custom terminal."
            )
            return False

        args = self._build_native_terminal_command(emulator, command_list)
        # startDetached returns immediately and lets the terminal outlive Baleno.
    ...
    def on_connection_ready(self, conn_type, client, channel, host_info,
        ...
        if conn_type == 'telnet':
            ...
            terminal_dialog.setWindowTitle(f"Baleno Terminal - {tab_label}")
    ...
    def connect(self):
        ...
        if self.debug_checkbox.isChecked():
            ...
            TerminalDialog._banner_index += 1
            terminal_dialog.banner_color = color
            terminal_dialog.setWindowTitle("Baleno Terminal - Debug Mode")
...
def main():
    ...
    app.setApplicationName("baleno")
    app.setDesktopFileName("baleno")
    ...
    icon_path = window.get_icon_path('baleno-256.png')
    if icon_path:
