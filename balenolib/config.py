"""Configuration management for Baleno."""
...
class ConfigManager:
    ...
    def __init__(self) -> None:
        if sys.platform == 'win32':
            ...
            self.config_dir = os.path.join(appdata, 'Baleno')
        else:
            # Get XDG config directory (defaults to ~/.config)
            xdg_config_home = os.environ.get('XDG_CONFIG_HOME')
            if not xdg_config_home:
                xdg_config_home = os.path.join(Path.home(), '.config')
            self.config_dir = os.path.join(xdg_config_home, 'baleno')

        # Create baleno config directory
