...
Baleno, which persists settings via ``ConfigManager`` JSON).  SQLite is
...
from balenolib.topology.models import TopologyGraph
...
def default_layout_path() -> str:
    """Return the default layout file inside the Baleno config directory."""
    xdg = os.environ.get('XDG_CONFIG_HOME') or os.path.join(Path.home(), '.config')
    return os.path.join(xdg, 'baleno', 'topology_layout.json')


def default_map_path() -> str:
    """Return the default full-map file inside the Baleno config directory."""
    xdg = os.environ.get('XDG_CONFIG_HOME') or os.path.join(Path.home(), '.config')
    return os.path.join(xdg, 'baleno', 'topology_map.json')
