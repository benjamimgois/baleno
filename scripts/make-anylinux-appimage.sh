#!/bin/bash
# Build a truly portable Anylinux AppImage for Baleno using quick-sharun.
#
...
VERSION="$(grep -oP '(?<=^VERSION = \")[^\"]+' "$SCRIPT_DIR/balenolib/constants.py" || echo "1.0")"
OUTNAME="Baleno-${ARCH}.AppImage"
...
echo "=== Baleno Anylinux AppImage Builder ==="
...
echo "[1b/7] Bundling balenolib into dist/baleno..."
...
#     script  = /tmp/.../usr/share/baleno/baleno
#
#   Inside AppDir (mounted AppImage):
#     AppDir/shared/bin/baleno → dirname x2 → AppDir
#     python3 = AppDir/bin/python3   (sharun hardlink → bundled Python)
#     script  = AppDir/usr/share/baleno/baleno
echo "[3/7] Compiling native launcher..."

cat > "$BUILD_DIR/baleno-launcher.c" << 'LAUNCHER_EOF'
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[])
{
    ...
    /* shared/bin/baleno  →  dirname x2  →  AppDir root  */
    char *p1 = strdup(self);
    ...
    snprintf(script, PATH_MAX, "%s/usr/share/baleno/baleno", app_dir);
...
gcc -O2 -march=x86-64 -o "$BUILD_DIR/baleno-bin" "$BUILD_DIR/baleno-launcher.c"
echo "  Compiled: $BUILD_DIR/baleno-bin ($(file "$BUILD_DIR/baleno-bin" | cut -d: -f2 | xargs))"
...
mkdir -p "$PREFIX/usr/share/baleno/assets/icons"
mkdir -p "$PREFIX/usr/share/baleno/assets/vendors"
mkdir -p "$PREFIX/usr/share/applications"
mkdir -p "$PREFIX/usr/share/icons/hicolor/256x256/apps"

cp "$BUILD_DIR/baleno-bin"              "$PREFIX/usr/bin/baleno"
cp "$SCRIPT_DIR/dist/baleno"            "$PREFIX/usr/share/baleno/baleno"
cp "$SCRIPT_DIR/assets/icons/"*.svg      "$PREFIX/usr/share/baleno/assets/icons/"
cp "$SCRIPT_DIR/assets/vendors/"*.svg    "$PREFIX/usr/share/baleno/assets/vendors/"
cp "$SCRIPT_DIR/baleno.desktop"         "$PREFIX/usr/share/applications/baleno.desktop"

# Icon: use PNG from assets if available
if [ -f "$SCRIPT_DIR/assets/icons/baleno.png" ]; then
    cp "$SCRIPT_DIR/assets/icons/baleno.png" \
       "$PREFIX/usr/share/icons/hicolor/256x256/apps/baleno.png"
else
    # Convert SVG to PNG as fallback
    _svg=$(ls "$SCRIPT_DIR/assets/icons/baleno"*.svg 2>/dev/null | head -1)
    if [ -n "$_svg" ]; then
        rsvg-convert -w 256 -h 256 "$_svg" \
            -o "$PREFIX/usr/share/icons/hicolor/256x256/apps/baleno.png" 2>/dev/null || \
        cp "$_svg" "$PREFIX/usr/share/icons/hicolor/256x256/apps/baleno.png"
    fi
fi
...
export APPDIR
export ICON="$PREFIX/usr/share/icons/hicolor/256x256/apps/baleno.png"
export DESKTOP="$PREFIX/usr/share/applications/baleno.desktop"
export OUTPATH="$SCRIPT_DIR"
export OUTNAME
export DEPLOY_PYTHON=1
export DEPLOY_QT=1
export DEPLOY_OPENGL=1
export MAIN_BIN="baleno"
# Tell lib4bin where to find libraries
export LIB_DIR=/usr/lib

# Use existing display (we're in a graphical session)
# The C launcher will exec python3 → baleno --help, giving strace all Qt libs
_DISPLAY="${DISPLAY:-:0}"
...
"$BUILD_DIR/quick-sharun" "$PREFIX/usr/bin/baleno" -- \
    "$PREFIX/usr/bin/baleno" --help 2>/dev/null || \
"$BUILD_DIR/quick-sharun" "$PREFIX/usr/bin/baleno"
...
install -dm755 "$APPDIR/usr/share/baleno/assets/icons"
install -dm755 "$APPDIR/usr/share/baleno/assets/vendors"
cp "$SCRIPT_DIR/dist/baleno"           "$APPDIR/usr/share/baleno/baleno"
cp "$SCRIPT_DIR/assets/icons/"*.svg     "$APPDIR/usr/share/baleno/assets/icons/"
cp "$SCRIPT_DIR/assets/icons/"*.png     "$APPDIR/usr/share/baleno/assets/icons/" 2>/dev/null || true
cp "$SCRIPT_DIR/assets/vendors/"*.svg   "$APPDIR/usr/share/baleno/assets/vendors/"
...
# Custom AppRun — calls bundled python3 (sharun hardlink) with the baleno script
...
# Users who want native Wayland: QT_QPA_PLATFORM=wayland ./Baleno.AppImage
export QT_QPA_PLATFORM="${QT_QPA_PLATFORM:-xcb}"
...
exec "$APPDIR/bin/python3" \
     "$APPDIR/usr/share/baleno/baleno" "$@"
APPRUN_EOF
