#!/bin/bash
# Build a portable Anylinux AppImage for Baleno inside an Ubuntu 22.04
...
IMAGE_TAG="baleno-appimage-builder:latest"
NO_CACHE=""
...
echo "=== Baleno Docker AppImage Builder ==="
echo "Engine  : $ENGINE"
...
echo "      Output AppImage → $SCRIPT_DIR/Baleno-x86_64.AppImage"
echo ""
...
"$ENGINE" run --rm \
    ...
    -e DISPLAY=:99 \
    -e OUTPATH=/output \
    -e OUTNAME="Baleno-x86_64.AppImage" \
    "$IMAGE_TAG" \
    bash -c '
        set -euo pipefail
        ...
        if ls /build/Baleno-*.AppImage 1>/dev/null 2>&1; then
            cp /build/Baleno-*.AppImage /output/
            echo ""
            echo "✓ AppImage copied to host: /output/Baleno-x86_64.AppImage"
        else
...
echo ""
echo "=== Build complete! ==="
if [ -f "$SCRIPT_DIR/Baleno-x86_64.AppImage" ]; then
    echo "AppImage : $SCRIPT_DIR/Baleno-x86_64.AppImage"
    echo "Size     : $(du -sh "$SCRIPT_DIR/Baleno-x86_64.AppImage" | cut -f1)"
    echo ""
...
echo ""
echo "Run:  chmod +x Baleno-x86_64.AppImage && ./Baleno-x86_64.AppImage"
